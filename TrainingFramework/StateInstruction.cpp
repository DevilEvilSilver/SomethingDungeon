...
StateInstruction::StateInstruction(void) {
	this->Init();
}
...
void StateInstruction::Init() {
	ResourceManager::GetInstance()->Init(FILE_R_INSTRUCTION);
	SoundEngine::GetInstance()->Init(FILE_SD_INSTRUCTION);

	//read data file
	FILE* dataFile;
	dataFile = fopen(FILE_S_INSTRUCTION, "r");
...
void StateInstruction::UpdateControl(float frameTime)
{
	//Button Quit
...
void StateInstruction::GetRenderOrder() {}